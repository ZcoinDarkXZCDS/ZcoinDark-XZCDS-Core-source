...
UniValue gobject(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp  ||
        ...
        throw std::runtime_error(
                ...
                "  vote-conf          - Vote on a governance object by masternode configured in zcoindark.conf\n"
    ...
    */
    ...
    if(strCommand == "vote-conf")
    {
        ...
        if(!fMnFound) {
            ...
            resultsObj.push_back(Pair("zcoindark.conf", statusObj));
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* ZcoinDark features */
    { "zcoindark",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "zcoindark",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "zcoindark",               "gobject",                &gobject,                true,  {} },
    { "zcoindark",               "voteraw",                &voteraw,                true,  {} },
