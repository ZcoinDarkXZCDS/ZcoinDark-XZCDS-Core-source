...
const char * const BITCOIN_CONF_FILENAME = "zcoindark.conf";
const char * const BITCOIN_PID_FILENAME = "zcoindarkd.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            if (mapMultiArgs.count("-debug")) {
                ...
                // "zcoindark" is a composite category enabling all ZcoinDark-related debug output
                if(ptrCategory->count(std::string("zcoindark"))) {
...
#else
    const char* pszModule = "zcoindark";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\ZcoinDark
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\ZcoinDark
    // Mac: ~/Library/Application Support/ZcoinDark
    // Unix: ~/.zcoindark
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "ZcoinDark";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/ZcoinDark";
#else
    // Unix
    return pathRet / ".zcoindark";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty zcoindark.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override zcoindark.conf
