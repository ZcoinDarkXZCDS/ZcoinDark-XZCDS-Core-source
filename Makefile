...
pkgdatadir = $(datadir)/zcoindarkcore
pkgincludedir = $(includedir)/zcoindarkcore
pkglibdir = $(libdir)/zcoindarkcore
pkglibexecdir = $(libexecdir)/zcoindarkcore
...
CONFIG_HEADER = $(top_builddir)/src/config/zcoindark-config.h
CONFIG_CLEAN_FILES = libzcoindarkconsensus.pc share/setup.nsi \
...
am__DIST_COMMON = $(srcdir)/Makefile.in \
	$(srcdir)/libzcoindarkconsensus.pc.in \
	...
	$(top_srcdir)/src/config/zcoindark-config.h.in \
...
ACLOCAL = ${SHELL} /tmp/ZcoinDark/build-aux/missing aclocal-1.15
AMTAR = $${TAR-tar}
AM_DEFAULT_VERBOSITY = 0
AR = /usr/bin/ar
AUTOCONF = ${SHELL} /tmp/ZcoinDark/build-aux/missing autoconf
AUTOHEADER = ${SHELL} /tmp/ZcoinDark/build-aux/missing autoheader
AUTOMAKE = ${SHELL} /tmp/ZcoinDark/build-aux/missing automake-1.15
AWK = gawk
BDB_CFLAGS = 
BDB_CPPFLAGS = 
BDB_LIBS = -ldb_cxx-4.8
BITCOIN_CLI_NAME = zcoindark-cli
BITCOIN_DAEMON_NAME = zcoindarkd
BITCOIN_GUI_NAME = zcoindark-qt
BITCOIN_TX_NAME = zcoindark-tx
...
COPYRIGHT_HOLDERS_FINAL = The ZcoinDark Core developers
COPYRIGHT_HOLDERS_SUBSTITUTION = ZcoinDark Core
COPYRIGHT_YEAR = 2018
CPP = gcc -E
...
MAINT = 
MAKEINFO = ${SHELL} /tmp/ZcoinDark/build-aux/missing makeinfo
MAKENSIS = 
MANIFEST_TOOL = :
MINIUPNPC_CPPFLAGS = 
...
OTOOL = 
OTOOL64 = 
PACKAGE = zcoindarkcore
PACKAGE_BUGREPORT = https://github.com/zcoindarkpay/zcoindark/issues
PACKAGE_NAME = ZcoinDark Core
PACKAGE_STRING = ZcoinDark Core 0.12.3
PACKAGE_TARNAME = zcoindarkcore
PACKAGE_URL = https://zcoindark.org/
...
ZMQ_CFLAGS = 
ZMQ_LIBS = -lzmq
abs_builddir = /tmp/ZcoinDark
abs_srcdir = /tmp/ZcoinDark
abs_top_builddir = /tmp/ZcoinDark
abs_top_srcdir = /tmp/ZcoinDark
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /tmp/ZcoinDark/build-aux/install-sh
...
pkgconfig_DATA = libzcoindarkconsensus.pc
...
DIST_CONTRIB = $(top_srcdir)/contrib/zcoindark-cli.bash-completion \
	       $(top_srcdir)/contrib/zcoindark-tx.bash-completion \
	       $(top_srcdir)/contrib/zcoindarkd.bash-completion \
...
COVERAGE_INFO = baseline_filtered_combined.info baseline.info \
  leveldb_baseline.info test_zcoindark_filtered.info total_coverage.info \
  baseline_filtered.info rpc_test.info rpc_test_filtered.info \
  leveldb_baseline_filtered.info test_zcoindark_coverage.info test_zcoindark.info

OSX_APP_BUILT = $(OSX_APP)/Contents/PkgInfo $(OSX_APP)/Contents/Resources/empty.lproj \
  $(OSX_APP)/Contents/Resources/bitcoin.icns $(OSX_APP)/Contents/Info.plist \
  $(OSX_APP)/Contents/MacOS/ZcoinDark-Qt $(OSX_APP)/Contents/Resources/Base.lproj/InfoPlist.strings
...
src/config/zcoindark-config.h: src/config/stamp-h1
	@test -f $@ || rm -f src/config/stamp-h1
	@test -f $@ || $(MAKE) $(AM_MAKEFLAGS) src/config/stamp-h1

src/config/stamp-h1: $(top_srcdir)/src/config/zcoindark-config.h.in $(top_builddir)/config.status
	@rm -f src/config/stamp-h1
	cd $(top_builddir) && $(SHELL) ./config.status src/config/zcoindark-config.h
$(top_srcdir)/src/config/zcoindark-config.h.in:  $(am__configure_deps) 
	($(am__cd) $(top_srcdir) && $(AUTOHEADER))
	rm -f src/config/stamp-h1
	touch $@

distclean-hdr:
	-rm -f src/config/zcoindark-config.h src/config/stamp-h1
libzcoindarkconsensus.pc: $(top_builddir)/config.status $(srcdir)/libzcoindarkconsensus.pc.in
...
$(OSX_APP)/Contents/MacOS/ZcoinDark-Qt: $(BITCOIN_QT_BIN)
	$(MKDIR_P) $(@D)
...
$(APP_DIST_EXTRAS): $(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZcoinDark-Qt
...
$(APP_DIST_DIR)/$(OSX_APP)/Contents/MacOS/ZcoinDark-Qt: $(OSX_APP_BUILT) $(OSX_PACKAGING)
...
#test_zcoindark.info: baseline_filtered_combined.info
#	$(MAKE) -C src/ check
#	$(LCOV) -c -d $(abs_builddir)/src -t test_zcoindark -o $@
#	$(LCOV) -z -d $(abs_builddir)/src
#	$(LCOV) -z -d $(abs_builddir)/src/leveldb

#test_zcoindark_filtered.info: test_zcoindark.info
#	$(LCOV) -r $< "/usr/include/*" -o $@

#rpc_test.info: test_zcoindark_filtered.info
...
#test_zcoindark_coverage.info: baseline_filtered_combined.info test_zcoindark_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zcoindark_filtered.info -o $@

#total_coverage.info: baseline_filtered_combined.info test_zcoindark_filtered.info rpc_test_filtered.info
#	$(LCOV) -a baseline_filtered.info -a leveldb_baseline_filtered.info -a test_zcoindark_filtered.info -a rpc_test_filtered.info -o $@ | $(GREP) "\%" | $(AWK) '{ print substr($$3,2,50) "/" $$5 }' > coverage_percent.txt

#test_zcoindark.coverage/.dirstamp:  test_zcoindark_coverage.info
#	$(GENHTML) -s $< -o $(@D)
#	@touch $@
...
#cov: test_zcoindark.coverage/.dirstamp total.coverage/.dirstamp

.INTERMEDIATE: $(COVERAGE_INFO)

clean-local:
	rm -rf coverage_percent.txt test_zcoindark.coverage/ total.coverage/ qa/tmp/ cache/ $(OSX_APP)
