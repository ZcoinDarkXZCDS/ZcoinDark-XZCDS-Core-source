...
*Setup instructions for a Gitian build of ZcoinDark Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the ZcoinDark
...
- [Building ZcoinDark Core](#building-zcoindark-core)
...
Clone the git repositories for ZcoinDark Core and Gitian.
...
in the ZcoinDark Core repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building ZcoinDark Core
----------------

To build ZcoinDark Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the ZcoinDark Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/zcoindark/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/zcoindarkpay/zcoindark
...
For example:
```bash
URL=https://github.com/crowning-/zcoindark.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit zcoindark=${COMMIT} --url zcoindark=${URL} ../zcoindark/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit zcoindark=${COMMIT} --url zcoindark=${URL} ../zcoindark/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit zcoindark=${COMMIT} --url zcoindark=${URL} ../zcoindark/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the zcoindark git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../zcoindark/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/zcoindark
SIGPATH=/some/root/path/zcoindark-detached-sigs

./bin/gbuild --url zcoindark=${BTCPATH},signature=${SIGPATH} ../zcoindark/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/zcoindark-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/zcoindark-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/zcoindark-osx-build.assert
```
...
[zcoindark/gitian.sigs](https://github.com/zcoindarkpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.
