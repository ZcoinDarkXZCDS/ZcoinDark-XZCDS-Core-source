...
ZcoinDark Core is a multithreaded application, and deadlocks or other multithreading bugs
...
- ThreadRPCServer : Remote procedure call handler, listens on port 4465 for connections and services them.
...
However, in open source software such as ZcoinDark Core, where everyone uses
...
pay attention to for reviewers of ZcoinDark Core code.

General ZcoinDark Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers
  ...
  - *Rationale*: ZcoinDark Core uses tinyformat, which is type safe. Leave them out to avoid confusion
...
be sent upstream but bugfixes may also be prudent to PR against ZcoinDark Core so that they can be integrated
